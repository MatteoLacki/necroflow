...
Implemented: `Pipeline.section(name)` labels subsequent node assignments with
pipeline-local presentation metadata. Graph JSON includes an unambiguous section
per node; PNG graphs group by sections only when all displayed rule calls have
one. Shared nodes with conflicting pipeline sections fall back to dependency
depth, and sections do not affect cache identity or execution.
